...
智能数据持久化和检索系统，为 AI 分析提供长期记忆支持

- **AI 分析存储**：自动保存所有 AI 分析结果和分析板块，支持时间范围查询
- **关键词统计**：追踪关键词热度趋势，记录跨平台出现情况
- **智能记忆管理**：创建和管理多种类型的记忆（每日总结、每周摘要、主题洞察等）
- **记忆关联**：建立记忆之间的关系链接（支持、矛盾、扩展、派生等关系）
- **高级查询**：支持按关键词、类型、时间范围等多维度搜索记忆
- **数据一致性**：基于 SQLite 的可靠存储，确保数据完整性

记忆系统类型：
- `DAILY_SUMMARY` - 每日新闻和趋势总结
- `WEEKLY_DIGEST` - 每周聚合洞察
- `TOPIC_INSIGHT` - 特定主题深度洞察
- `PATTERN` - 识别的模式和趋势
- `SIGNAL` - 重要信号和事件

> **💡 详细使用文档**：参见 [docs/memory_system_usage.md](docs/memory_system_usage.md)
>
> **🧪 测试覆盖**：80+ 单元测试和集成测试，确保系统稳定性
